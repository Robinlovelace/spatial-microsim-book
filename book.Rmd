...
```{r, echo=FALSE}
# *Reweighting with ipfp* ([](#ipfp)). # causing error in jekyll
```
...
. See the references listed in the Appendix
(
[](#further)
). As
...
demand for travel and individual characteristics for origins and destinations. Although the techniques for generating spatial microdata in transport are the
same in transport, the process is referred to as *population synthesis*.
...
It is good practice to to remove
...
individual-level data, you cannot do spatial microsimulation.
...
survey must have been completed equally by residents in
every geographical zone under consideration. It is important
...
a zone's populatin is not a good basis for
...
not percentages, and must contain sufficient
categories for each variable. The sleep dataset
...
constraint variables are best seen as a single variable.
...
The chi-squared statistic is defined as the sum of the square of the errors divided by the observed values (Agrest 2007):
...
because there are simply so many combinations of rows, zones and
variables to choose from.

This chapter tackles each of these issues. It is the first
chapter in the book so far that is departs from the starting
point of having access to spatial microdata. The methods
are equally applicable to synthetic spatial microdata as
for real microdata. With the open data movement, it is possible
that governments realise that the benefits of releasing more
geocoded individual-level data are likely to far outweigh any
potenial privacy issues (especially in an age of smartphones
which can track our every move). If and when such datasets
become available, all previous work to generate synthetic spatial
microdata will not be in vain: it will serve as a useful
...
research is could put pressure on governments to release more
individual-level information in a way that would not
comprimise security, ethics or morality.
