...
would be missing. We can try the algorithms proposed
in ([](#sintegerisation)). However, as we
can see with the two little following codes, this leads to
some errors regarding to the constraints.
...
This little errors are often very small and if
...
If desired, we can adapt TRS to make sure to 
fit the constraints at the end.
...
that we underevaluate the population.] the data and identify the missing individuals, in 
...
with the bigger decimal parts (as in TRS). Note that we chose
to adapt the `resTruncate` instead of defining
another tabular. The code works as followed: As long as there are missing
