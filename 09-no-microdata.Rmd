...
In summary, if you have no sample data, it is not such a big problem. 
The techniques can be adapted and you can nevertheless create a synthetic population.
We presented in this chapter two different methods, depending on the type of 
possible constrain input data. First, we developed a procedure when 
having global cross tables and local marginals. Secondly, we went through
a spatial microsimulation when having aggregate data at different aggregation levels.
