...
Still, the meaning of spatial microsimulation is still largely unknown. This is
...
is ambiguous and inconsistent about the method.
The situation is worsened by work that treats spatial microsimulation
as a magical black box or that does not define terms.
This ambiguity is partly because spatial microsimulation
...
This book encourages reproducibility by
providing the tools for its readers to
actually *do* spatial microsimulation on realistic
data. All the findings presented
in this book can be reproduced using code and data in the book's
Github repository:
...
and rigour. The book also aims to
open the possibilities of spatial microsimulation to more people, with
...
then; software is rarely discussed in papers on the subject.
...
The booklet is a precursor to a book
CRC Press's
...
Table: Geographically aggregated count data on age in SimpleWorld.
...
Table: Count data on sex in SimpleWorld.
...
Table: Individual-level survey data from SimpleData.
...
Table: Weight matrix reflecting how representative each individual from the survey
(rows) is relative to each zone (columns) in SimpleWorld.
...
This dataset contains a single row per individual (as with the
...
is as representative as possible of the aggregate constraints, whilst
...
in the table below. The complete spatial microdataset contains
a number of individuals for each zone, resulting in a more or less realistic
insight into the inhabitants of SimpleWorld that will be useful for modelling,
policy evaluation and other applications. 

Table: Synthetic spatial microdata from SimpleWorld.
...
The table is a reasonable approximation of the inhabitants of zone 2:
...
as a whole, and for each region. If the relationship between age and
sex is markedly different
...
Fortunately R is an accomplished tool for performing these tasks, as we shall see.
The next chapter builds on the data we load here to
run a spatial microsimulation model.
...
the datasets you have are not suitable for spatial microsimulation because
they lack sufficient constraint variables that are shared with
individual representative individual-level data. We assume that you have already
checked this. The checking process for the datasets used in this chapter is simple:
...
Constraint datasets are usually made available one variable at a time,
...
it is worth inspecting each object to
ensure that they make sense before continuing. Taking a look at `con_age`,
...
This tells us that there are 12, 10 and 11 individuals in zones 1, 2 and 3,
...
variables early, to avoid over-complicating and slowing-down the analysis.
...
[Appendix on subsetting](#subsetting).
...
Users should be aware that `cut` results in a vector of class *factor*, which
...
The dimensions
of at least one of these objects must change
...
This process, which can also be described as when
"each categorical (or factor)
variable is expanded into columns of indicator variables with one column allocated
to each level"

Second, `colSums()` is used to take the sum of each
...
Note that the second call to `model.matrix` is suffixed with `[, c(2, 1)]`.
This is to swap the order of the columns: the column variables produced
by `model.matrix` are in alphabetic order
whereas in the constraint object `cons`, `male` is to the left of `female`.
Such subtleties can be hard to notice yet may change the results
...
In this chapter we progress from preparing the input data: 
now it's time to run a spatial microsimulation model.
The three SimpleWorld datasets, loaded in the previous chapter,
are used. Being small and simple, the example enables understanding
the process on a 'human scale' and facilitates experimentation
without overloading your computer.
The methods apply equally to larger and more complex projects
so practicing the basic principles and methods presented here
will make subsequent steps much easier.
...
is using iterative proportional fitting (IPF).
In this section we will *reweight* the individual-level data loaded in
...
**ipfp** R package. The `ipfp` function runs the IPF algorithm
...
In the CakeMap example we use only the `ipfp` reweighting strategy
as it is more concise, generalisable and faster than
[IPF in pure R](#IpfinR). On a modern laptop, the `ipfp` method
was found to be *almost 40 times faster* than the 'IPFinR' method.
These tests can be run using the `microbenchmark()` commands found towards
the end of 'CakeMap.R'.^[The second of these benchmarks depends
on the old `smsim-course`, the repository of which needs to
be downloaded (from
[github.com/Robinlovelace/smsim-course](https://github.com/Robinlovelace/smsim-course))
and saved to the user's computer.]

As before, the output of the process is a matrix object entitled `weights`.
Other than 
...
Let's query the gender of the 4th individual, as an example
