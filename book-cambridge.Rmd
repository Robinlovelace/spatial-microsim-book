...
range of applications including housing, transport and
...
```{r, echo}
ipfp(cons[1,], t(ind_cat), rep(1, nrow(ind)), maxit = 20, verbose = T)[[1]][c(1:3, 9, 10, 11),]
```
...
(the rows of `cons`) is `rep(1, nrow(ind))`. This simply a series of ones.
...
weight variable we created earlier. (The reason for setting `maxit` to 3
and creating the object `weights_3` will become clear in the next section).

```{r}
weights_maxit_3 <- weights # create a copy of the weights object
for(i in 1:ncol(weights)){
  weights_maxit_3[,i] <- ipfp(cons[i,], ind_catt, x0, maxit = 3)
}
```
...
```{r}
ind_agg <- t(apply(weights, 2, FUN = function(x) colSums(x * ind_cat)))
```
...
Let's look at the first two zones (rows) for the first
three constraints:
...
```{r}
t(apply(weights_maxit_3, 2, FUN = function(x) colSums(x * ind_cat)))[1:2, 1:3]
```

Clearly the final weights after 3 iterations of IPF represent the constraint
variables well, but do not match perfectly. This shows the importance of
