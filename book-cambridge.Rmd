...
result selected one of each individual, plus a clone of individual 3.
It is important to emphasise that the result need not work out like this:
changing the the value inside the brackets proceeding `set.seed`
results in many other combinations of individuals being selected. This happens
because the method relies on *pseudo random numbers* to select values
...
*proportional probabilities* strategy is that completely unrepresentative
...
which ensured that any individual with a weight above 1 would be sampled
