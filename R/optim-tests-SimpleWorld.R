...
umat[1:ncol(ind_cat)][rep(1:4, umat$ind_num),] # we've returned full circle to the correct population
...
out$value
x <- c(as.integer(c(b,c,d,e)))
tae <- abs(colSums(ind_cat[a,]) - cons[1,])
...
set.seed(2014)
for(i in 1:5){
  tmp_res <- optim(par = rep(1, nrow(indu)), fn = fun, method = "Nelder-Mead", ind_num = indu, con = cons[1,], control = list(maxit = i * 100))
  ...
  tmp_res <- GenSA(par = rep(1, nrow(indu)), fn = fun, lower = rep(0, nrow(indu)), upper = rep(10^5, nrow(indu)), control = list(maxit = i), ind_num = indu, con = cons[1,])
  opt_res <- rbind(opt_res, c("GenSA", i, tmp_res$value, NA))
}

opt_res$fit <- as.numeric(opt_res$fit)
opt_res$maxit <- as.numeric(opt_res$maxit)
qplot(data = opt_res, maxit, fit, color = algorithm, geom="line") +
  ylab("Total Absolute Error") + xlab("Iterations") + scale_color_brewer(palette = 2, type = "qual")
