...
economic and data-collecting activities are centered. We will here develop results 
for Belgium in a specific study. Note that this
chapter is based on a research made by Dumont Morgane (UNamur) and funded by the 
...
-*Independent data (individuals and households)* ([](#IndData)) considers the
case in which data on households and individuals remain completely separate.
-*With additional household's data* ([](#AddData)) presents a strategy when
having additional data to the individual data.

Note that the first section explains a method of the litterature only 
theoretically, whereas the second sections is developed more in details
...
the fit. In the research of Johan Barthelemy, they executed these
exchanges by following the principles of a `tabu search`. This
consists in an algorithm that try exchanges and remember the last tries
to avoid making always the same exchange. For more information about `tabu search`: [@tabuSearch].
...
\caption{Illustration the problem of grouping members of married couples with children.}
\end{center}
\end{figure}
...
unpossible to consider a table of age differences per hair color if this
...
\hline
\end{tabular}
\caption{Example of an age distribution table for couples without children}
...
man of age 15-20. With this reasoning, we can calculate the new Table \ref{fictivageDistrib2}, 
...
\hline
Municipality & Woman's age & Man's age & Count & Proportion of men married \\
...
The methodology for the couples with male heads is illustrated in Figure 
\ref{AlgoCouple}. For
...
it will remain only few partners to assign. Thus, 
...
is empty, or there are no remaining partnerin the possible ages for the
remaining heads.
...
Figure \ref{NonAssigne} indicates that the
...
Figure \ref{BadSize} illustrates the proportion of individuals living 
with a wrong number of cohabitants. In the different municipalities, 
the proportion varies from 0.7\% to 1.05\%.
...
To summarise, we can group the individuals into households in all cases.
However, depending on the precision of the available data, these allocations
to households can be more or less close to the real population. The more pertinent information you
have, the more your algorithm can be adapted to generate more realistic households.
