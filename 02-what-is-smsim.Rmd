...
the various interpretations and uses of spatial microsimulation.
This should help understand the types of problem the approach is
best suited to solve.
The chapter is ordered as follows:

- Answers to the question 'what is spatial microsimulation and why use it?',
to provide a no-nonsense introduction to the field.
- A description of spatial microsimulation terminology (\ref{terminology}),
to explain the spatial microsimulation nomenclature in more detail.
This section shows how spatial microsimulation
can be understood as either a method or an approach
([](#meth-approach)),
how the methods can best be communicated to others ([](#comms)) and
the use of the term in this book ([](#this-book)).
- An exploration of the applications of spatial microsimulation in
the Applications section
 ([](#applications)).
- The question of what spatial microsimulation is not and how
it relates to other approaches such as ABM
([](#is-not)).
- The often unspoken assumptions underlying spatial microsimulation
methods ([](#assumptions)).

## What is spatial microsimulation and why use it?

For the purposes of this book spatial microsimulation is defined as:

> A set of methods for the creation, analysis and modelling of individual-level data allocated to geographical space.

Spatial microsimulation is therefore a *multi-level* approach.
It is well-suited to the analysis of complex phenomena which play-out over
geographical space, such as transport systems and the housing system.
Because it includes the creation of synthetic data, the method is well-suited
to situations in which available data are limited.

Spatial microsimulation is a way to combine the advantages of individual-level
data with the geographical specifity of geographical data. If used correctly,
...
to avoid confusion. Furthermore, it is important to understand something about
how others have used 'spatial microsimulation' to avoid
misinterpretation of literature that employs the term.

There are valid reservations about the term *spatial microsimulation*, as
described below
(see [](#is-not)).
However, *spatial microsimulation*
it is an appropriate label for the material covered in this
book because it is
...
and observes their God-like influence on the virtual citizens. 
The analogy of SimCity to describe spatial microsimulation.
...
In some ways the comparison is appropriate:
...
Feynman's distinction between a thing's label what
...
remembering that the audience may
have not have heard of spatial microsimulation before
(see [](#comms)).
...
been used to tackle. Before us pause to consider how spatial
...
But it does not necessarily involve imply interaction between these individuals.
For this,
...
However, many of the ABMs presented in the academic literature lack realism,
instead assigning illustrative (and in some cases arbitrary) values to the
agents, that may not closely correspond to reality, and area where the population
synthesis phase of spatial microsimulation can be of great use.
...
```{r, fig.cap="Schematic of population synthesis, a critical element in spatial microsimulation", fig.scap="Schematic of population synthesis", fig.align='center', echo=FALSE}
...
described in [@Tomintz2008], the outputs of such a model
...
[@sutherland2013euromod]. The majority of microsimulation
...
with an agent based modelling approach (described in chapter 13),
