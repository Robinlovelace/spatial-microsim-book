...
This book is for anyone who wants to learn how to *do* spatial microsimualtion.
By that we mean taking individual-level and geographical data and
processing them to solve real-world problems. This is a practical book.
...
microdata, for example in a module contributing to an MSc in Transport Modelling,
...
the two chapters in
Part III assumes that the reader is competent with generating and handling spatial
microdata, as introduced in chapters 4 and 5. These are the central Chapters in
the book in terms of generating spatial microdata with R and are probably the
...
microsimulation in the first place. Chapter 2, by contrast, brings the
...
Strict conditions inhibiting data access and use are aimed to protect citizen
...
initial conditions for) agent-based models (see Chapter \ref{ABM}).
...
inconsistency about it means.  Worse is work that
...
thousands of packages that extend R's capabilities by providing new functions and
improvements are being added all the time. This book seeks to provide an
up-to-date overview of the functions from mature packages for spatial microsimulation
at the time of publication. It is a fast-moving game, however, so it is worth
searching the internet for updates to existing packages and new packages for
spatial microsimulation. New software can save both research and computer time
...
new users become familiar with the language (see figure 4.1). 
...
This document concerns a book that will still be improved
after edition. Any comments, relating to code, content or
...
Glossary at the end of the book may help to clarify what they mean. Chapters
4, 5, 6 and 7 encapsulate the process of *population synthesis* which
generates spatial microdata and which is
key to much spatial microsimulation work. They should be read together in that order.
Chapters 8 introduces the important step of model validation.
Chapter 9 and 10 cover more advanced methods for generating spatial microdata when
...
([](#alternative-reweighting)) describes some alternative methods, beyond IPF, for population
