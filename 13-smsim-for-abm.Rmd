...
The RNetLogo package ([citation]) allows us to control a NetLogo model directly from R. Now that we have a working model and have explored it in NetLogo to get a feel for what it's doing (and make sure that it is doing what we want it to do), we can explore the model more systematically, and for the most part do not need the graphical interface anymore. RNetLogo allows us to run the model silently in the background (in so called *headless mode*), issue commands, set global values, repeatedly run the model and collect the results. 
