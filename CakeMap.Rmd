...
The whole process first needs to load the data. This is done in a similar way 
that for the previous section. However, an additionnal step is to make the categories
of `cons` and `ind` correspond. For example, for the car ownership, `ind` contains "1" 
and "2", but `cons` contains "Car" and "NoCar". To facilitate the procedure, we uniformize 
these names to the ones that give the information without having to find the keys ("1"="Car";
"2"="NoCar"). We keep "Car" and "NoCar", "f" and "m" for the sex, etc.
This code is included in the 'CakeMapMipfpData.R' file.

```{r}
source("R/CakeMapMipfpData.R")
```

The data are loaded and if we print the first lines of `ind`, the category 
are now the same as the one in `cons`. Note that this step was not 
necessary for the `ipfp` package, because the user has to encode
the constraints in the right order and only the order has to be respected.
However, having the same identification of the same category is 
always more clear for validation and interpretation. Moreover, 
the structure of some constraints will have to change and 
this process will be easier if the names are similar.
...
take the cross table of the individual data. It is important to notice
...
To use the whole power of the `mipfp` package, we
chose to add the spatial dimension into the weight matrix
instead of performing a `for` loop over the zones.
For this purpose, we repeat the table for each area.
We define the names of the new dimension and we 
create an array with the good cells and names.
...
A good point with `mipfp` is that the algorithm
first check the consistency of the constraint. If 
the total number of individuals in a zone is not exactly
the same from one constraint to another, `mipfp` will
return the warning : 'Target not consistents - shifting to probabilities!'
In this case, the cells of the resulting table are probabilities, 
such as the sum of all cells is 1. This means that the algorithm does not
know how many people per zone you want. Thus, you just have to 
determine the total population and multiply the probabilities by this number.
...
contains in total less individuals and the answer to this question 
could be "NA", we consider the totals of the 
two first constraints as the good one.

To perform an effective (and not probability) `mipfp`,
we need to re-scale `con3`, to keep its proportions, but
with the good totals per zone. 
...
The second and third constraints are simple marginals per zone, whereas
the first is equivalent to a cross tabulated constraint per zone
(sex and age). The structure of the first constraint has to be different 
to execute `mipfp`. 

```{r}
# Print current structure of con1
head(con1)
```

Thus, the structure needed is a three dimensional array, which
dimensions are (zone, sex, age). This conversion consists in 
taking the cells of `con1` in the right order and including them
inside a three dimensional array. This code is included into the 
file 'CakeMapMipfpCon1Convert.R' and the resulting array is called
`con1_convert`.
...
All constraints being well reexpressed and the margins being
re-scaled, the definition of the target and its description
can be done. The target is the set of constraints and the description 
...
The remaining step consists in executing the `Ipfp` function 
on these data.

```{r}
weight_mipfp <- Ipfp( weight_init, descript, target, 
                      print = FALSE,tol=1e-12)
```

Remark that we don't print the `tol` in each iteration, because
more than 400 iterations are necessary to reach convergence.
...
a category and a zone, are not so good. The analysis of 
the quality and the comments on this generation are in the next chapter.
Here, we focus now on the comparison between `ipfp` and 
`mipfp` for CakeMap.

## Comparison of the two methods

We mentionned in the `ipfp` part that the three constraints
do not contain the same number of individuals. To compare 
both methods, we will apply a second time `ipfp`, but 
with the rescaled constraints. Note that in case of 
non consistency constraints, `mipfp` warns the user, 
but `ipfp` not. Thus, with `ipfp`, it is very 
recommanded that the user check its constaints'totals.
Remark that instead of number of iterations, we 
chose to fix the tolerance threshold to $10^{-12}$,
such as for the `mipfp`.

```{r}
# Reperforming ipfp with the rescaled constraints
cons_prop <- cbind(con1, con2, con3_prop)
weights <- apply(cons_prop, 1, function(x) ipfp(x, ind_catt, x0, tol = 1e-12))
...
The structure of both resulting tables are different. 
...
weights matrices (maximum of differences being of order $10^{-12}$).
...
To visualize all the weights, we chose to plot 
...
resulting weights, we discover that the biggest
...
The results are the same. Thus, it is logical to prefer
the version that we be executed the faster. For this reason, 
the time is an important characteristics to compare
the packages. 

First, when performing the two generation with the initial example
of CakeMap, `mipfp` was slower than `ipfp`. However, 
when thinking of the conception of the algorithms, `ipfp` will take
more time when the length of the survey choosed as microdata increases.
This phenomena should not appear in the `mipfp` package. To prove this
intuition, we decide to take CakeMap with variable individuals 
in the micro data. To have an idea of what happens beyond the 916
persons in the data, we copy the database a second time.
The code used is in the "CakeMapTimeAnalysis.R".
...
It confirms that for problems with few individuals in the microdata, 
...
In the best case, to perform to a microspatial simulation, 
you have a big microdata and you use `mipfp`. Note that
when having very few microdata, it can be better to 
consider a method without microdata. Like this, you avoid to bias
the result with the category included in the microdata.
The representativity of the microdata is important. 