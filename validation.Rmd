...
map_pack <- c("ggmap", "rgdal", "maptools", "rgeos", "dplyr")
...
This ends our analis of correlation. Next we can calculate
...
investigation. An early strategy to characterise this zone is to visualise.
...
just above Leeds city centre. This does not help us identify or solve the root
source of the problem, but it does provide useful context that could help our
investigations to improve the model.
...
```{r, echo=FALSE, fig.cap="Geographical distribution of Total Absolute Error (TAE).", message=FALSE, warning=FALSE, results=F}
...
ggmap(get_map(bbbig, maptype = "terrain")) +
  ...
  # geom_path(data = fwards, aes(long, lat, group = group), color = "white") +
