...
this validation is the only available test of the model's output,
...
An important remark on these two kinds of validation is taht it does not
test the model in the same "level". Indeed, internal validation tests
the model quality, considering the assumptions and the input data as 
pertinent, correct and representative. If this validation fails, you may
have a problem in the choice made during the modelisation, the algorithm, or 
you have contradictory constraints, so no solution exists. It is really a
method problem. In all cases, if internal validation fails, the reasoning
to proceed to the model has to be reviewed. 

On the other side, the external 
validation makes a comparison with other databases, researchs. This tests
simultaneously if the modelisation process is good, the input data are 
correct and representative and the assumptions to the model are realistics.
If this one fails, the problem can come from everywhere. This is why both 
are really interresting together. 
...
can sounds complicate, but it is just a standardized covariance. If the fit is perfect,
both vectors (simulated and constraint) evolve in the same rythme, so they have a high covariance
...
which simply show that the individual-level dataset has
...
We have just calculated our first goodness-of-fit measure for a spatial microsimulation model and the results are encoraging: the high correlation suggests that the model is working: it has internal consistency and could be described as 'internally valid'. Note that we have calculate the correlation 
before integerisation here.
...
```{r, echo=FALSE}
# (MD, 2015) Check the formula (really only for con1?)
# add the results for cakemap and interpret
# add some representations of the errors 
```
