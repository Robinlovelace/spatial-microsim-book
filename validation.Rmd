...
Note that the maps presented in Figure 7.1 looks identical for TAE and SAE values
except for the scale; the similitude of these measures of fit can be
varified using a simple correlation:

```{r}
cor(TAEVec, SAEVec) # note that the two measures are very correlated
```
...
In the example of CakeMap, no databases are available to check if the consumption of cakes
in our simulation is pertinent with other databases. In the case of microsimulation, 
it is always hard to proceed to an external validation, because in practice, when we have 
databases, we use them for the simulation. Thus, comparing the result with this data
is included in the process of internal validation.
