...
which(CorVec == min(CorVec))

# second minimal value
min(CorVec[-c(84)])
```

This ends our analyse with the correlation. Now, we can calculate another measure of goodness of fit.
We can develop total absolute error (TAE),that is easily defined as a function in R:
...
By applying this function for CakeMap, we find a TAE of 26445.57, as coded below. This can
sound very big, but remember that this measure is very dependent on the scale of the problem.
Indeed, an error for 26445 persons out of 4,871,397 is not a so big. For this reason, we
would like now to calculate the standardised absolute error (SAE). We observe a SAE of 
only 0.54%. Note that here, we considered the whole population, so we have to divided
...
TAEVec <- rep (0, dim(cons)[1])
SAEVec <- rep (0, dim(cons)[1])
...
# identify the worst zone
which(TAEVec == max(TAEVec))
which(SAEVec == max(SAEVec))
