...
In a beautiful world where scientists could access to all databases and all possible information, 
for example about individuals, their characteristics, their habits, their movements and everything, 
we could study a wide range of aspects of the real world, the life, ... However, 
in most of the cases, reseachers have to analyse entities, but they have no precise 
data about it. How could we predict the evolution of a part of the population
if we do not know who are these people and their specific characteristics.

Spatial microsimulation give us an idea, a statistical approximation, of the data we need to start 
a lot of research in many fields. This topic will always be needed, because we could 
never know and observe the state and characteristics of everything at each time.

Spatial microsimulation is a statistical technique for combining individual and
geographical data, but also census and sample data. The resulting *spatial microdata* are useful in many
...
because R being opensource, everybody can write a new package. So, we have to check
the quality of a used package before to use it as a "black box".

R is a powefull statistical language and its interface "RStudio" really helps new users
to become familar with this language. 
