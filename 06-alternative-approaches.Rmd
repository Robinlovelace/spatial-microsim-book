...
This chapter describes briefly other techniques for spatial microsimulation.
We have different methods, each corresponding to a section : 

-*GREGWT* ([](#is-not)) contains a method based on a Generalized Regression Weighting 
procedure.
...
A simplified version of this algorithm (and other algorithms) is provided in
by @Rahman2009.  The the algorithm is described in more detail in @Tanton2011.
...
The code below
uses this implementation of the GREGWT algorithm, with the data from the
Chapter 3.
...
Intuitively, we search the number of occurrences we want of each individual to
...
Thus, we want to find a vector of parameters `w` that will multiply the `indu` matrix,
which is similar to `ind_cat`, but each row of `ind_cat` represents an individual of the sample,
whereas each row of `indu` concerns a type of individual (if 2 persons in the sample has the
same characteristics, the line will appear only once). The cells of `indu` contain
the number of time that this kind of individual appears in the sample. We want the result
of this multiplication to be as close as possible to the constraints.
...
definition of an optimisation problem, our `par` are the theoretical $x$. 
...
```{r, fig.cap="Relationship between number of iterations and goodness-of-fit between observed and simulated results for different optimisation algorithms", fig.scap="Total Absolute Error results for several algorithms", fig.width=4, fig.height=4, fig.align='center', echo=FALSE}
...
algorithm in function of the number of iterations (Figure \ref{TimeOptim_GenSA}). This figure
...
take less time. Until, it appears that IPF is the best in terms of iterations
and the time needed for few iterations is good.

```{r, fig.cap="Relationship between processing time and goodness-of-fit between observed and simulated results for different optimisation algorithms",fig.scap="Total Absolute Error in function of processing time for several algorithms", fig.scap="Relationship between processing time and goodness-of-fit", fig.width=4, fig.height=4, fig.align='center', echo=FALSE}
...
Combinatorial optimisation (CO) is a complete field consisting in a different kind of
methods to result optimisation problem. Instead of having one candidate that
...
depending on how we chose the combination of candidates and how we determine
...
to be calculated, CO having to perform an evaluation for each candidate,
the algorithm can take a long time to run.
...
very effective when the objective function has several local minimum and
...
provided by **simPop** and example of one of **simPop**'s functions.
...
Note that this package gives several alternatives. It contains "model-based methods,
calibration and combinatorial optimization algorithms" 
(as we can see on the help of this package).
...
software as alternative synthetic population generator.
...
In summary, this chapter presented several alternative to IPF
to perform to a spatial microsimulation. We mentioned a regression
method : GREWT, ways using the optimization theory, an R package
containing several procedures : *simPop* and the UDST, currently
developed in python.
