...
```{r, echo=FALSE}
# (MD) Pay attention. Linking variable and constraint variable are potentially different
# (MD) In the aggregated data, you can have variables that are not in the individual level data.
# (MD) of course, they are then not used as 'constraints', but the additional variables
# (MD) could be assigned to the final individuals in a stochastic way, usinf conditional probabilities
```
...
-   A **linking variable** is a variable that is shared between individual and 
    ...
    **constraint variables** because they *constraint* the weights for individuals
    in each zone.
