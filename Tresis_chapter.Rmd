---
title: "The TRESIS approach to population synthesis and spatial microsimulation"
layout: default
output:
  pdf_document:
    fig_caption: yes
bibliography: tresis.bib
---

# The TRESIS approach to population synthesis and spatial microsimulation[^authors]

```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
library(knitr) # Needed for the kable function used in this chapter.
```
[^authors]: Contributed chapter by Richard B. Ellison and David A. Hensher, Institute of Transport and Logistics Studies, The University of Sydney

TRESIS is a Transport and Environment Strategy Impact Simulator originally developed by Professor David Hensher and others at the Institute of Transport Studies (now Institute of Transport and Logistics Studies --- ITLS) that evolved from an earlier project for the Bureau of Transport and Communications Economics [@Hensher2002a]. 
The TRESIS software provides users with the ability to assess the impacts of a large number of both transport and land-use policies including on the environment, end-user costs and benefits and government revenue.
TRESIS incorporates an extensive model system including discrete and continuous choice models that is combined with a process of population (or more accurately, household) synthesis and a set of equilibriation criteria to provide a powerful alternative to some more widely used microsimulation implementations.

This chapter introduces the TRESIS model system before providing a detailed description of the generation of synthetic households implemented in R. This is followed by details of how the synthetic households can be used to identify where households live using a residential location model.

## Overview of TRESIS model system
The TRESIS model system was designed to cover many of the long-term and short-term decisions made by individuals and households that affect how they travel.
These range from the very long-term (e.g., where to live and where to work) to the very short-term decisions (e.g., departure time and travel mode).
Together, these models predict households' long-term decisions and how these decisions then affect how individuals within that household travel given user-specified transport and land-use policies.
The figure below shows an overview of the high-level TRESIS model structure [@Hensher2008].
![Overview of TRESIS model structure](figures/TRESISModels.png)

The TRESIS demand models have been estimated from a combination of stated preference (SP) and revealed preference (RP) data using the NLOGIT software.
The model system provides for feedback between the models that are connected using inclusive value parameters that are fed up through the models.
In this chapter, we discuss a stand-alone residential location model but it is important to understand that in the full TRESIS model system, the residential location choice model is closely linked to the other models including the fleet size, vehicle type choice, workplace location and dwelling type choice models.

### Differences between TRESIS and other microsimulation systems
TRESIS differs from standard micro-simulation models in that TRESIS has been designed to be primarily a strategic model system that preserves the behavioural richness in the models.
In contrast, standard micro-simulation techniques (used in transport) are largely designed for small-scale studies that cover only a small area.
The TRESIS models also operate at a slightly higher level than most micro-simulation systems since individuals in the household are not run through a micro-simulation model to determine specific routes.
Instead, the TRESIS system applies the estimated demand based on the travel demand models and uses this within a network assignment model.
Although this means that link-level travel times for a specific individual can not be determined, it provides a more reliable aggregate estimate of travel times that is needed for the efficient running of the TRESIS models.
...
These synthetic households form the basis of the TRESIS model system and provide the necessary underlying variables required for the models in the simulation.
...
#### How do synthetic households in TRESIS differ from traditional population synthesis
In contrast to traditional population synthesis (described in earlier chapters), synthetic households in TRESIS have several primary differences.
One of these differences is that synthetic households retain their original composition and characteristics from the source sample.
This means that rather than take individual-level microdata and assigning these individuals to a household, individuals remain linked to their original households.
Crucially, it is the household as a single entity that is the unit of anaylsis/synthesis within TRESIS.
Furthermore, each household is not simply a sum of the household members but may also be considered to have its own characteristics.

Another of the main differences between the synthetic households as used in TRESIS compared to traditional population synthesis is that synthetic households as used in TRESIS are not assigned to a specific geographic zone.
Instead, the sample from which the synthetic households are drawn is limited to those households that are resident in the whole of the study area (generally a city's Greater Metropolitan Area) resulting in synthetic households having an overall geographic coverage matching the study area but no specific location within it.
...
The specific location of each synthetic household is assigned by running them through the TRESIS model system which, among other models, includes a model for the choice of residential location using more disaggregate zones.
It must be emphasised that the household weights of the household do not change when the household is assigned to a zone.

### Required data for generating synthetic households
Generating synthetic households requires a combination of micro-level census data and aggregate population statistics.
The microdata needs to include matched household and individual level data to allow for the retaining of the link between individuals and households within the synthetic households.
The aggregate population data need to include the variables that are of interest for selecting synthetic households but is only required as a total for the whole study area.
This data should ideally be available as a multi-way cross-tabulation for each combination of variables that are desired for representing the households in the population.
The reasons for this will be explained further later in this chapter.
In Australia, for which TRESIS was developed, this data is typically all available from the census but could come from different sources if the variables are comparable.

### Synthetic households in R
Generating synthetic households in R requires three stages.
The first stage involves ensuring that the variables in the micro-data sample are compatible with those used in the demand models.
...
Finally, the micro-data is sampled to generate the final synthetic households each with an individual weight based on their incidence in the population.
...
```{r}
set.seed(99) # This line sets the seed for the random number generation in the 
# sample() function used below. This is used here to ensure that the results are 
# consistent for each run. To ensure that your weighted of synthetic 
# variables is reasonable for your application it is best
# to run this several times without a seed and compare the results.

# Randomly select the sizes of the households and then assign the
# unique IDs of each household to sequential individuals in the dataset.
ind$hhld <- rep(1:1000, times=sample(c(1:6), 1000, replace=TRUE))[1:nrow(ind)]
...
hhlds <- group_by(ind, hhld) %>% 
  summarise(
    numres = n(),
    # Values of 8 and 97 mean "long-term unemployed and not answered respectively"
    ...
    # Take the midpoint of the age ranges then take the maximum for that household
    maxage = max(rowMeans(read.table(text=str_replace(ageband4, '-', ','), 
                                     header=FALSE, sep=','))),
    # Value of 1 means they have a car, value of 2 means they do not
    # Divide the number of cars by 1.5 to get a more realistic number since more than 1 person
...
The _cons_ dataframe from the CakeMap dataset contains the aggregate population-level data we need for generating the weights and identifying the required sample.
However, since the _cons_ dataframe is at the Ward level, we need to aggregate to get a total for all the wards.
...
However, since the crosstabs are not available for the CakeMap data, for the purposes of this example we will assume that the distributions of the two variables (car and professionals) are the same for all combinations as they are for the population.
...
prop.table(c(Professionals=totcons['X1.2'], 
             OtherOcc=sum(totcons[c(match('X1.1', names(totcons)), 
                                    match('X2', names(totcons)):match('Other', names(totcons))
                                    )])))
```
...
This is used here to avoid hard coding the correct index which we may change inadvertantly.
...
```{r}
popdisttot <- round(popdistprop*(totcons['Car']+totcons['NoCar'])/3)
popdisttot
```
...
```{r}
synhhlds <- sample(filter(hhlds,numprof > 0, numcars > 0)$hhld, popdist['Car','Professionals'])
synhhldweights <- rep(popweights['Car','Professionals'], popdist['Car','Professionals'])
synhhldtypes <- rep('Car+Professionals', popdist['Car','Professionals'])
```
...
The third line is used just to help identify in which group each household was sampled from.
...
This is then repeated for each of the subsequent groups.

```{r}
# Households with a car and no professionals
synhhlds <- c(synhhlds,
              sample(filter(hhlds,numprof == 0, numcars > 0)$hhld, popdist['Car','Other'])
              )
synhhldweights <- c(synhhldweights,
                    rep(popweights['Car','Other'], popdist['Car','Other']))
synhhldtypes <- c(synhhldtypes,
                  rep('Car+Other', popdist['Car','Other']))
# Households with a professional but no car
synhhlds <- c(synhhlds,
              sample(filter(hhlds,numprof > 0, numcars == 0)$hhld, popdist['NoCar','Professionals'])
              )
synhhldweights <- c(synhhldweights,
                    rep(popweights['NoCar','Professionals'], popdist['NoCar','Professionals']))
synhhldtypes <- c(synhhldtypes,
                  rep('NoCar+Professionals', popdist['NoCar','Professionals']))
# Households with no car and no professionals.
synhhlds <- c(synhhlds,
              sample(filter(hhlds,numprof == 0, numcars == 0)$hhld, popdist['NoCar','Other'])
              )
synhhldweights <- c(synhhldweights,
                    rep(popweights['NoCar','Other'], popdist['NoCar','Other']))
synhhldtypes <- c(synhhldtypes,
                  rep('NoCar+Other', popdist['NoCar','Other']))
```
...
```{r}
synhhldDF <- data.frame(hhld=synhhlds,hhldweight=synhhldweights,hhldtype=synhhldtypes)
```
...
# Add an ID for each individual in the household (starting from 1 for each household)
...
So we can avoid regenerating synthetic households everytime we change our residential location model, it is useful to save our data frame to an RData file:
...
The models used in TRESIS are linked meaning the model for residential location can not be completely separated out from the rest of the models.
...
The models in TRESIS were estimated using the NLOGIT software package because it provides features needed for advanced models unavailable in other packages.
...
The model below was estimated based on using average values for each ward using a very simple model and can not be regarded as a "good" model.
However, it fits the required structure for the utility expressions and in the absense of disaggregate data for estimating the model we will use it for this example.
...
resmodelout <- round(resmodel$coefficients,4)

```

$$
\begin{aligned}
U_1 &= `r str_replace_all(str_replace_all(paste(resmodelout[names(resmodelout)[grep("1:",names(resmodelout))]],str_replace(names(resmodelout)[grep("1:",names(resmodelout))],"1:",""),sep='*',collapse = " + "),'[*][(]intercept)',""),"[+][ ][-]","- ") ` \\
U_2 &= `r str_replace_all(str_replace_all(paste(resmodelout[names(resmodelout)[grep("2:",names(resmodelout))]],str_replace(names(resmodelout)[grep("2:",names(resmodelout))],"2:",""),sep='*',collapse = " + "),'[*][(]intercept)',""),"[+][ ][-]","- ") ` \\
U_3 &= `r str_replace_all(str_replace_all(paste(resmodelout[names(resmodelout)[grep("3:",names(resmodelout))]],str_replace(names(resmodelout)[grep("3:",names(resmodelout))],"3:",""),sep='*',collapse = " + "),'[*][(]intercept)',""),"[+][ ][-]","- ") ` \\
U_4 &= `r str_replace_all(str_replace_all(paste(resmodelout[names(resmodelout)[grep("4:",names(resmodelout))]],str_replace(names(resmodelout)[grep("4:",names(resmodelout))],"4:",""),sep='*',collapse = " + "),'[*][(]intercept)',""),"[+][ ][-]","- ") `
\end{aligned}
$$
...
The _with()_ function allows the variable names in the utility expressions to be used as-is, without prepending the data frame name and a \$ to them and not attaching the dataframe to the global environment.
...
while (i <= 4) {
  synrun[,7+i] <- with(synrun,
       eval(parse(text=c(
         ...
         "6.5633 - 1.3535*prof + 26.8106*mgmt - 0.8307*rout - 3.1094*numCars",
...
The final probablities for 10 households are shown in the table below.
...
However, even from this relatively simplistic example, it is apparent that even within the same groups, the probabilties of living in a particular zone vary (sometimes substantially).
...
Calibration is not a straight-forward process but involves adjusting the alternative-specific constants in the model until the estimated shares match the population shares within an acceptable level of tolerence.
...
$$
\begin{aligned}
U_1 &= 3.2989*mgmt + 1.8519*rout + 1.733*numCars \\
U_2 &= 5.5633 - 1.3535*prof + 26.8106*mgmt - 0.8307*rout - 3.1094*numCars \\
U_3 &= 0.115 - 1.0771*prof + 1.3984*mgmt + 1.781*rout + 1.9871*numCars \\
U_4 &= 3.7052 - 8.0874*prof + 33.2434*mgmt + 1.8073*rout - 1.291*numCars
\end{aligned}
$$
...
\begin{aligned}
U_1 &= 3.2989*mgmt + 1.8519*rout + 1.733*numCars \\
U_2 &= 4.5633 - 1.3535*prof + 26.8106*mgmt - 0.8307*rout - 3.1094*numCars \\
U_3 &= 0.115 - 1.0771*prof + 1.3984*mgmt + 1.781*rout + 1.9871*numCars \\
U_4 &= 3.7052 - 8.0874*prof + 33.2434*mgmt + 1.8073*rout - 1.291*numCars
\end{aligned}
$$
...
### Limitations
The primary limitation of the TRESIS approach to population synthesis is also one of its key strengths.
The heavy reliance on detailed disaggregate household and population-level data means that acquiring the necessary data needed for generating the synthetic households, and estimating and calibrating the models is frequently a time-consuming and sometimes expensive undertaking.
However, once the data has been collected, the resulting model system is extremely behaviourally powerful allowing predictions not only of the current state, but also of future forecasts of decisions under uncertainty provided the variables on which the forecasts rely are available in the synthetic households and models.
...
However, there is ongoing development within the Institute of Transport and Logistics Studies to develop freight and light commerical vehicle models.
...
In contrast to the passenger models that rely on synthetic households, the freight and commerical vehicle models have applied the concept used for generating synthetic households, to firms resulting in the development of "synthetic firms".
...
### MetroScan-TI
TRESIS, although extremely powerful in many ways, has lacked a few features that are increasingly being needed for a full assessment of potential transport and infrastructure projects.
...
This chapter has provided an introduction to the TRESIS approach to population synthesis through the use of synthetic households and demand models.
The relative simplicity, yet behavioural richness, of the synthetic households means they are relatively straightforward to implement in R (subject to data requirements) and can be used for a variety of applications not limited to those involving transport models.
