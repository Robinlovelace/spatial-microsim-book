...
# The TRESIS approach to population synthesis and spatial microsimulation^[Contributed chapter by Richard B. Ellison and David A. Hensher, Institute of Transport and Logistics Studies, The University of Sydney Business School, The University of Sydney]


TRESIS is a Transport and Environment Strategy Impact Simulator originally developed by Professor David Hensher and others at the 
...
TRESIS incorporates an extensive model system including discrete and continuous choice models. These are combined with a process of population (or more accurately, household) synthesis and a set of equilibriation criteria.
TRESIS provides a powerful alternative to some more widely used microsimulation implementations.

```{r, echo=FALSE}
# Add reference to end of this paragraph (RL)
# 
```

This chapter introduces the TRESIS model system from a spatial microsimulation perspective. The bulk of substantive material focusses the generation of synthetic households, which is described in detail. This is the first time that TRESIS has been implemented in R. The methods and code blocks oultined below could therefore provid a stepping stone towards further transport modelling work R, which is currently under-used in transportation research. The chapter demonstrates how the synthetic populations generated through TRESIS can be used to identify where different types of households live using a residential location model. The chapter concludes with discussion of other important aspects of transport modelling that could be integreted into R, building on this work.

## Overview of TRESIS modelling system

The TRESIS modelling system was designed to cover many of the long and short-term decisions made by individuals and households that affect how they travel.
...
In this chapter, we discuss only the stand-alone residential location model. This is just one part of TRESIS, which would be closely coupled to other sub-models covering fleet size, vehicle type choice, workplace location and dwelling type choice [@Hensher2008].

### Differences between TRESIS and other microsimulation systems

TRESIS differs from standard micro-simulation models in that it was designed primarily it preserves the behavioural richness of observed individuals.
In contrast, standard micro-simulation techniques commonly used in transport are largely designed for small-scale studies that cover only a small area.
TRESIS tends to operate at higher geographical levels than micro-simulation systems. Individuals in the household are not run through a micro-simulation model to determine specific routes.

Instead, the TRESIS system estimates travel demand based on a separate travel demand models within a network assignment model.
Although this means that link-level travel times for a specific individual can not be determined, it provides more reliable aggregate estimates of travel times. These are needed for the efficient running of the TRESIS models.
...
Household are seen as a single entity,
the unit of anaylsis within TRESIS.
...
a total count for zones and the whole study area.
...
the input data for TRESIS are all available from the census in most advanced
nations such as Australia, where the model was first developed.
