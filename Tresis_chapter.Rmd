...
This chapter introduces the TRESIS model system from a spatial microsimulation perspective. 
The bulk of substantive material focusses on the generation of synthetic households, which is described in detail. 
This is the first time that a method for implementing TRESIS' synthetic households in R has been published. 
The methods and code blocks outlined below could therefore provid a stepping stone towards further transport modelling work in R, which is currently under-used in transport research. 
...
The chapter concludes with discussion of other important aspects of transport modelling that could be integreted into R, building on this work.

## Overview of TRESIS modelling system

The TRESIS model system was designed to cover many of the long and short-term decisions made by individuals and households that affect how they travel.
...
These inclusive value parameters provide the mechanism through which the decicions made using one model (e.g., what vehicle to buy) are incorporated into the decisions made using other models in the system (e.g., where to live).
...
Although this means that link-level travel times for a specific individual can not be determined, it provides more reliable aggregate estimates of travel time across the whole network. 
...
Households are seen as a single entity, and used as the unit of anaylsis for many of the models, including the residential location model, within TRESIS.
...
The TRESIS approach uses the same synthetic households, and the individuals within those households, as were used in this chapter for the residential locaiton model, to estimate the destinations and modes of trips.
...
Although developing the full set of models can not be described as a simple process, as demonstrated in this chapter, the functionality of R, and some of its packages, makes it well suited to applying transport models.
