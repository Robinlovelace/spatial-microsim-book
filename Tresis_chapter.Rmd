...
```
[^authors]: Contributed chapter by Richard B. Ellison and David A. Hensher, Institute of Transport and Logistics Studies, The University of Sydney Business School, The University of Sydney

TRESIS is a Transport and Environment Strategy Impact Simulator originally developed by Professor David Hensher and others at the Institute of Transport Studies (now Institute of Transport and Logistics Studies --- ITLS) that evolved from an earlier project for the Bureau of Transport and Communications Economics [@HensherD2002;@Hensher2002a]. 
...
The TRESIS models also operate at a slightly higher spatial level than most micro-simulation systems since individuals in the household are not run through a micro-simulation model to determine specific routes.
...
The specific location of each synthetic household is assigned by running them through the TRESIS model and equilibrating the model system at the trip, location and vehicle type levels in order to reproduce the base year spatial travel and location profiles as well as vehicle type and fleet size composition and which, among other models, includes a model for the choice of residential location using more disaggregate zones.
...
In Australia, for which TRESIS was developed (for six capital cities), this data is typically all available from the census but could come from different sources if the variables are comparable.
