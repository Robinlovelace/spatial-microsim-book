...
using R via RStudio. The aim is twofold: 1) to produce
...
inputs.  R is an accomplished tool for data reformatting,
...
```{r studio,  fig.cap="The RStudio interface with the 'spatial-microsim-book' project loaded",fig.scap="The RStudio interface", fig.align='center', echo=FALSE, message=FALSE}
...
`c(0, 20, 40, 60, 80, 120)` to continue the age example) are
preferable.
\index{breaks}
\index{bins}
...
loosing any information at this stage because we have saved the original
...
in this chapter.
The following check-list outlines the main features of the input datasets
