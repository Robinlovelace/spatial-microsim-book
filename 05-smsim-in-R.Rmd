...
The final stage that often accompanies integerisation is expansion.
Thic can be achieved by a function, which we call `int_expand`:
...
Let's test the new function on our integerised data:
...
```{r, echo=FALSE}
# TODO: explain this function in more detail! (RL)
```
...
for(i in 1:nrow(cons)){
  ints <- int_expand(int_trs(weights[, i]))
  ints_df <- rbind(ints_df, data.frame(id = ints, zone = i))
}

# Method 2: using apply
ints <- unlist(apply(weights, 2, function(x)
  int_expand(int_trs(x)))) # integerised result
