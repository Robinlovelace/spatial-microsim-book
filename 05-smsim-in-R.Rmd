...
# Discuss population synthesis without an input microdataset, with reference to Barthelemy and Toint (2013)
...
are provided recent papers by Lovelace and Ballas (2012) and, in the
...
**ipfp** and **mipfp** packages (which is available on-line: all R packages are open source),
they can at first seem like "black boxes" which
...
The first step before to perform to an algorithm is to initialize each needed object.
...
all weigths were initialised to 1 and the new weights equal
the coefficients.
...
(Figure 5.2). After constrainint only by age, the simulated cell counts 
...
it helps understand how IPF works and aides diagnosing issues with
...
The first randomly choses the individuals, with a probability proportional
to the weights.
The second is 'Truncate Replicate Sample' [@Lovelace2013-trs]
and is described in
([](#sintegerisation)).
...
making the results difficult to use as an input to agent-based models.
...
Simply rounding the the weights is one integerisation method,
...
each fractional weight to plus or minus one, and is known as TRS:
...
Lovelace and Ballas (2012) tested 5 of these and found that the probabilistic
