...
'Truncate Replicate Sample' [@Lovelace2013-trs], described in ([](#sintegerisation)).
...
This also implies the possibility to chose an individual more times than
...
**mipfp** must be used (described in ([](#nomicrodata))). Moreover, 
...
to have more realistic weights : integers. After that, a section contains
the process to transform the weights into a final table containing
all generated individuals with their characteristics.

Finally, we compared the two current developed packages to do IPF and 
conclude that **mipfp** is more general and which one to use will
depend on the context and the kind of input you have.
