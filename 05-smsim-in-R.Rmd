...
individuals to zones. Other methods can acheive the same result,
as discussed in the first section of this chapter and as
demonstrated in the next chapter (\ref{alt}).
...
IPF, **ipfp** and **mipfp** in sections \ref{ipfp} and \ref{mipfp}
...
experimentation with small datasets to avoide
...
create the spatial microdata is expansion. This can be achieved 
...
each cell will contain the ID of the sampled individual
of the sample to take. This is illustrated in the code below,
which uses the integerised weights for zone 1, generated above. 
...
weights of `Ipfp` into individual data, using `int_expand_array()`
on the integerised weights (see section \ref{})
