...
will have a weight of 'representatively' of the zone. The weights matrix 
...
weight matrix, then you integerise it.
...
population and a zone for each person.
...
and individuals with high weights must be sampled
...
Note that $w_1$ is a vector of same length. Then, $int(w_1)$ will be the result
...
process of *integerisation* in a systematic way to maximize the fit between
...
However, we can reexecute the same code and obtain different 
results:
...
two missing individuals by chosing two cells that will be incremented.



### Expansion

As illustrated in figure 5.1., the remaining step to obtain 
a final spatial microdata is the expansion. This can be achieved 
by this function, which we call `int_expand_vector`:
...
This function returns a vector, which element represents
an individual in the final spatial microdata and 
each sell will contain the ID of the individual
of the sample to take. This is illustrated with 
the `int_weight1` integerised SimpleWorld weights for
zone 1. 
...
int_expand_vector(int_weight1)
```

The final data can be found simply 
by replicating the individual, thanks to 
the indices found.

```{r}
# Generate spatial microdata for zone 1
ind_full[int_expand_vector(int_weight1),]
```
...
a final spatial microdata has to be perform
for all zones. This section gives concise ways
to reach the final data. Note that if you skipped the
detailed previous sections, you will need 
to load the functions by executing `source("R/functions.R")`.

Two strategies for doing this for all zones 
...
*Ipfp*. As with the package **ipfp**, we can chose a stopping criterion defined by *tol* and/or a maximum
...
The function `int_trs` presented in the **ipfp** section is 
...
integers close to the weight. This process has
always a maximal error of 1 per category.
...
used in the **ipfp** section, because the 
structure of data is totally different.
Indeed, with **ipfp** we simply need to
replicate the individuals, where in the 
**mipfp** case, we have to retrieve 
...
is an dataframe, which rows represent individuals.
The first column is the gender, the second is age 
and the third is the education level. We print
the head of this dataframe and we can check that
...
Able to deal with bigger problems, the package **mipfp** presented here can also manage to 
...
Indeed, the version with `apply()` calls `n_zones` times the function
...
cells. Instead of having `n_zones` different little matrices,
