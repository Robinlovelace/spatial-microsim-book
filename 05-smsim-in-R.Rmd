...
maths that underlie the method (section \ref{ipftheory}) before implementing
the algorithm in base R (section \ref{ipfinr}). Then, two packages that automate
IPF, **ipfp** and **mipfp** in subsequent sections
([](#ipfp) and [](#mipfp) respectively). The chapter also introduces the important concepts of 
...
The aim is to obtain the final weight matrix that will represent how well each individual 
fits each zone in terms of their characteristics. Each row of this matrix is an individual and each column 
