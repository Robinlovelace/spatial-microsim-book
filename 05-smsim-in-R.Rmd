...
output stored in 'long' format represented in the right-hand box of Figure5.1.
...
weights <- matrix(data = 1, nrow = nrow(ind), ncol = nrow(cons))
weights1 <- weights2 <- weights # create additional weight objects
...
has an eductational level of 3 or 4. This is as expected.
...
\caption[Distribution of a vector generated randomly]{Distribution of a vector generated randomly (each integer between 1 and 100,000 being equally likely).}
\end{figure}
...
this would add much complixity and computational 
...
The result, stored in `weight_mipfp`, is a `data.frame` including
the final weight matrix.
We can check the margins and print the final 
weights. Note that income is still not present in the matrix.
We look at the result for the aggregate
