...
This chapter focuses on the mininum input datasets needed for the spatial
...
below 
([](#Selecting)).
Focussing only on variables of interest will ensure you do not
...
unnavailable at any level (e.g. hair length, if your research question related
...
that there are many ways to load the data and to "clean" it. So, we present 
here a detailed process in the case of the data avaible with this book.
Some parts of the explanation here are really specific to this example.
However, if you want to use your datasets, you could perhaps follow a 
similar process. In each cases, the last section of this chapter consists
in a list of things you have to check by loading your datasets.
...
mistake with a real data is that the order in which zones are loaded changes from
...
sequence allowing different databases to be correctly joined. Moreover,
that will eventually allow the attribute data to be
...
interest, the following command could be used to update the object. Only the relevant variables corresponding to
...
However, these datasets cannot possibly be compared because rows and columns does not 
represent the same information, so in many cases they are of different dimensions:
...
before they can be pertinently and easily compared. To do this
we 'flatten' the individual-level dataset;
this means increasing its width
to match the constraint data. This is really intuitive, because it 
corresponds to express this dataset in a similar way to the aggregate-level table.
Each category of each variable will become a column with boolean number. Indeed,
for each row (so, each individual), the columns corresponding to his own characteristics
will be 1 and 0 for the other columns.
...
## Checking your process to load the data

Here are an abstract of some points that are really important to have loaded the data correctly :

1. Constraint and target variables have been loaded separately : you have one dataset
    which rows correspond to an individual and another which rows corresponds to a 
    spatial zone;
2. The categories of the common variables are identical and their names also :
    for example, if you have the exact ages on one side and a category on the
    other side, the spatial microsimulation can not occur;
3. The structure of both datasets have to be the same : the columns are the categories
    of the variables. The individual datasets contains boolean numbers and the aggregate one
    contains the number of persons in this zone having this characteristics;
4. The desire total number is define and correspond to the sum of all categories 
    for each constraint variable.

Note that in the majority of the research requiring the analyse of datasets, the collection, 
cleaning and loading of the data is a part that take more than the half of the study period.
