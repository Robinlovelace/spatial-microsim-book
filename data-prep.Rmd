...
# Data preparation with R {#data-prep}
...
individual-level survey data, can be loaded and 'cleaned' in R. It is the
first practical chapter, so it will also get you up-to-speed with
using R via RStudio for spatial microsimulation. The aim is to produce
data that is in a form ready for population synthesis and to acquaint
people who are not experienced R users with the software and a workflow
...
modelling process. The limiting factor for model fit in many cases will
...
individual-level data generating a synthetic population is more difficult. 
...
contain 5 categories may look good on paper. However, if those categories are
...
`c(0, 20, 40, 60, 80, 120)` to continue the age example) are preferable.
...
`cut(x = c(20, 21, 90, 35), breaks = c(0, 20, 40, 60, 80, 120))`. The result
is a `factor` variable
as follows: `(0,20]   (20,40]  (80,120] (20,40] `. This output uses standard
...
$0 < x \leq 20$. Note the `]` symbol means 'including'.)


but if on closer inspection those 
categories predominantly define very small subsets of the population, 
...
Real-world individual-level are provided in many formats.
...
are resilent to differences in the number of individuals according to each
...
spatail microsimulation; for a more general introduction to data formatting and
cleaning methodology see Wickham (2014). To summarise this information, 
the last section of this chapter provides 
a checklist of items to ensure that the data has been adequately cleaned
...
(e.g. because the *population bases* are different) this is problematic (see
the Glossary for a description of the population base).
