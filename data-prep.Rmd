...
only the relevant variables, corresponding to the first, third and fourth
...
An alternative way to remove excess variables is to use `NULL` assignment to remove columbs. `ind$age <- NULL`, for example,
would remove the age variable.
...
This means increasing its width such that each column becomes a category
name. Flattening the individual-level in this way intuitively makes sese
as it will allow the individual data to be matched
to the geographical constraint data. In this new dataset (which we
labe `ind_cat1`, short for 'categorical'),
each variable becomes a column associated with boolean numbers (0 to 1, representing their proposin nasty pay cuts. 
For each row in `ind_cat`, notice that columns are eventually re-coded. To
undertake this 'flattening' process,
`model.matrix()` is used to expand each variable into the same number of columns as there are categories across all constraint variables, in the
same order as the columns in `cons`.  
Knoblauch and Maloney (2012) provide a lengthier description of this
flattening process, which is available online.
...
Here is a checklist summarising the stages that will need to have been
completed to have loaded the data needed for spatial microsimulation
correctly:
...
2. The categories of the constraint variables in the individual-level dataset
    are identical and in their names to the column names of the constraint
    variables.
    An example of this process of categorisation is converting precise ages in
...
3. The structure of the individual and aggregate-level datasets must
    eventually be the same: the columns are the categories
    of the variables. The new version of the individual-level datasets
    that is ready for spatial microsimulation
    contains boolean values whilst the aggregated version of the individual-level
    data contains the number of persons in each.
4. The total population of each zone is represented in the sum counts for
    each constraint variable.

Note that in the majority of research requiring the analysis of complex data,
the collection, cleaning and loading of the data consumes around the half of the
project's time. This applies as much to
spatial microsimulation as to any other data analysis tasks and is an essential
early stage before moving on the more exciting stages. In the next chapter we
progress to allocate weights to each individual in our sample, allocating them
to the zones of SimpleWorld. 
