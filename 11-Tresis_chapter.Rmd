...
This chapter presents an alternative approach to spatial microsimulation that involves both the
generation and simulation of individuals in household units. The method, known as TRESIS, has
been developed for many years at the University of Sydney. TRESIS is mature and well-tested and,
in-line with its development at the Institute for Transport and Logistics Studies (ITLS), is especially
well-suited to solving transport-related problems.

This is the first published implementation of TRESIS in R. It does not use a package of its own
(although the development of a **TRESIS** package in R would be possible, building on the following code).
...
research ([](#tresis-over)); a demonstration of the generation of realistic households used in
TRESIS, building on CakeMap input data ([](#tresis-house)); and an illustration of how demand
models can be used to allocate these synthetic households to geographical zones ([](#tresis-alloc)).
...
data. TRESIS is a relative simple approach. This, combined with
...
(subject to data requirements). TRESIS
and can be used for a variety of applications not limited to those involving transport models.
