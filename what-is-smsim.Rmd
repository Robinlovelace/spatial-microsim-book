...
possible to the analysed population and with a disaggregate spatial information. In most
of the cases, we use it for allocating individuals from a survey dataset
to administrative zones, based on shared variables between the areal and
individual level data. This can also add missing characteristics to each individual 
(already allocated to a spatial zone) with the help of a survey.
...
microsimulation is, and what it is not, at the outset. However, in each cases, 
this term means reaching data with a spatial disaggregated level and 
generated with an approximation method, but not coming directly from a census
of the whole desired population.
...
with the term favoured by the majority of practitioners. A lot of 
scientific terms denote a set of methods that have a common aim. 
Just keep in mind that spatial microsimulation is needed in very many 
fields, so each specialist has adapt it to his research. This ambiguity
points out the strong usefullness of this concept.
...
to an uninitiated audience. Let alone describing Gibb’s sampling. That is 
a constant challenge for the scientists. Using a specific term when the audience
is experimented and simplify and explain with simple words when not, this is
the wealth of scientific researchers.
...
with available data. Then, we can see "micro" evoquing the desire to simulate
the data in a individual level. Indeed, we do not have interest on simple marginal
information, but on a high precision data where all the individual are known with 
their specific characteristics. Finally, "spatial" is there to indicate that
each individual is allocated to a zone, that depending on the context
can correspond to a municipality, a district, a country and so on. Most of the 
time, these zones are geographical.

So, in each applications, we speak about a population (in statistical sense, so 
a set of entities that we analyse). For example, a population can be a set
...
#[Morgane: It is normal to have 2 times 'transport'?
#In transport same in health perhaps? Or I don't know what you wanted to tell here.]
...
step to represent the individuals' evolution over time. However, the time is not 
really analysed. Spatial microsimulation finds a population with available data and 
apply it with data from 2000 and then from 2001 the whole process has to be remake.
Agent based modelling, by contrast imply some kind of interaction between the individual units
and the time is an integral part of the process. The aims of the two methods are complementary.
ABM consider having an initial population and making it evoluate through time and space 
depending on some rate and interaction between the individual, while spatial microsimulation
generate a population from data with lacks at a given time. Spatial microsimulation can be 
...
As we have seen, the term "spatial microsimulation" is ambigous and regroup 
a wide range of methods with a similar aim. In this book, we consider the
most used part of this branch, called reweighting. This consider that you 
have a part of data about a whole population (from a census) and another
part about only a part of the population (from a survey).
