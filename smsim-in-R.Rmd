...
The R package  **mipfp** came after **ipfp**
and consist in a more generalized version of the IPF algorithm.
Indeed, **ipfp** generates a two-dimensional weight matrix based on mutually
...
**mipfp** allows multiple cross-tabulations in the constraint variables,
such as, for example, age/sex and age/class.
...
**mipfp** provides a multidimensional implementation of the iterative 
proportional fitting procedure (ipfp). This is able to easily estimate 
...
**ipfp** package which was designed to solve only the 2 dimensional case.

This difference is explained by the initial aim of the packages. 
**mipfp** was specially design to solve problem of spatial microsimulation, 
whereas **ipfp** was created to solve an algebra problem of the form  $Ax=b$.
...
The `Ipfp` function of **mipfp** fills a $N$-dimensional weight 
matrix based on a range of aggregate-level constraint table options. 
In other words, `Ipfp` is  more flexible than the previously mentioned
approaches; it is relatively easy to adapt 
the code for diverse applications. The first step is to load the package
into the workspace:
...
Assume that we want to determine the contingency table 
...
Note that we will consider the variables in the following 
...
former is written in C, while the latter is written in R. With **ipfp**,
some coding and manipulations of 
data has to be performed to consider more than two variables or to use
cross-tabulated constraints. This is explained by the fact that
this package was initially designed to resolve linear problem of
...
the same characteristics could appear only once and have a starting weight of 2
...
We have constraints on sex and age. **mipfp** will define the number
of persons in each category so that the resulting contingency table respects all constraints.
...
To execute an IPF, we need a starting table.
For SimpleWorld, a microdata is available. We can start with 
this table and then iterate the IPF to obtain
the final contingency table (sex x age) thanks to the formula
...
Now, the current matrix fits exactly the age constraint. However, 
some differences appear between the current and the desired sex totals.
We will proceed to a similar reasoning for the sex constraint.
...
in the form of contingency table. We just 
follow the same logic, but by watching populations in terms of tables.

When having this table completely available and efficient, we know how many
individuals we need in each category. However, the weights are fractional.
To reach a final individual dataset, we can consider the weights as probabilities
...
Both methods do a similar simulation. They have to use the same algorithm.
We will first verify it by comparing the results.Then, we will measure the times used 
...
Now, we solve the SimpleWorld 
problem with **mipfp**. We proceed zone per zone. To explain the process, the generation 
of the first zone is here explained. The constraints for this area are : 
...
The inputs of `Ipfp` have to be included in a list. We chose to 
consider the age first and then the sex. Remark that we have to 
take the sex data in the other order to be coherent with the
seed. It is really important to check the order of the category,
because if the seed has (Male, Female) and the constraint (Female, Male), 
the algorithm will inverse the constraints. 
...
The available sample establishes the seed of the algorithm, also known as 
the initial weight matrix. We can take into account the entire individual level
data, after checking that the categories are matching.
Thus, we keep the separation between ages 
under and above 50. We also consider the income information, even if it 
does not appear in the constraints. This simply means that this information
will be stored. It is also possible to not consider the income here and
...
The result, stored in the `weight_mipfp` is a dataframe including
...
for a comparison purpose, we look at the result for the aggregate
...
apply(weight_mipfp$xi.hat, c(1,2), sum)
```


The results are fractional, as the one found with **ipfp**. 
Now that the process is well understood, we can generate the weight
matrix for each zone. For each area, the initial weight matrix will be the same.
We chose to store all weights in an array `Mipfp_Tab`, which first dimension represents
the zone.

```{r}
# Initialising the result matrix
Names <- list(1:3, colnames(con_sex)[c(2,1)], colnames(con_age))
Mipfp_Tab <- array(data = NA, dim = c(NbZones, 2 , 2), 
                   dimnames = Names)
...
for (zone in 1:NbZones){
  ...
  Mipfp_Tab[zone,,] <- apply(res$xi.hat,c(1,2),sum)
}
```

This piece of code has performed an IPF for each zone. 
We can print the result for zone 1 and observe that it correspond
...
constraints, we could also consider the zone as a variable. Like this, 
the execution of only one `Ipfp` generates the whole result. 
...
#Mipfp_in_one <- array(data = NA, dim = c(NbZones,2,2,5))

#for (i in 1:NbZones){
#  Mipfp_in_one[i,,,] <- weight_init
#}
...
Remark that the output of the function `Ipfp` can be used 
directly to perform steps such as integerisation and expansion
or as probability.
However, for this example, we would like to verify that both algorithms
calculated the same weights. For this, one output has to be transform 
to fit the structure of the other function's output. We here
adapt the `Ipfp` result to the `ipfp` final matrix. 

The aim is here to transform the output
into a form similar to the one generated in the section **ipfp** 
to allow an easy comparison. The comparable table will be called 
...
Now, we can fill in the matrix `weights_mipfp`.
...
```{r}
# Loop on the zones to transform the result's structure
for (zone in 1:NbZones){
...
The simulation of SimpleWorld with **mipfp** is now comparable to the one with **ipfp**, 
since we generated weights for each individual in each zone. Having
the same structure, the integerisation and expansion step explained for **ipfp** could
...
results being retranscribed to the same form, we can compare them.
We observe that the bigger difference is of the order
...
individual with **ipfp** and for each different category with **mipfp**
(that can be retranscribe in the other form if needed).

To evaluate the efficiency of both methods, we will 
consider the computational time of both functions. 
...
we perform the same calculus. Thus, the times are comparable.
