...
Cross-tabulated constraints can be used and generated in **mipfp**.
This means that we can consider the zone as a variable, allowing
population synthesis to be performed with no `for` loop,
generating a result for every zone. 

Indeed, we just have to create a weight matrix with one 
additional dimension : the number of the zone. Being an
supplementary dimension, we can access the table for one
...
times as the number of zones. And for the structure, 
we chose to have the variable in the order (zone, age, sex, income).
Note that we add the zone as first dimension here, but we could
chose to put it in second, third or last as well.
...
Now, we have a bigger matrix, containing `n_zone` times more 
cells. To access the whole data about the first zone, 
we have to type `mipfp_zones[1,,,]`. For the first area, 
we can check that the corresponding cells equal the previous
initial weights.
...
The 20 cells of zone 1 equal the previous matrix. With this initial 
weights, we can keep the whole age and sex constraint tables
(after checking the order of the categories). The constraint 
of age is in this case a cross-tabulated constraint with as first
...
We have to adapt the `target` and the `descript` variables. 
The new target takes the two whole matrices and the descript
...
This process gives exactly the same result as the for loop.
This is more practical and efficient, but here, in terms of
comparison, to be equitable we compare the result for each 
zone turn by turn.
