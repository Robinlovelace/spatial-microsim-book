...
To illustrate the concept further, the aim of aircraft design is to minimise
weight by changing its shape, and materials. But these modifications
must proceed subject to some constraints, because the
airplane must be safe and and sufficiently voluminous to 
...
the weight ($f_0$ which is a single value depending on $x$). But this search
must take place under
volume (depending on the shape) and safety constraints
...
aircraft design, we must define the problem to optimise and then
set th constraints.  
