...
NbCatAge <-ncol(con_age) #number of different categories for the var "age"
NbCatSex<- ncol(con_sex)  #number of different categories for the var "sex"
```
...
```{r}
# Assign values to the previously created weight matrix to adapt to age constraint
for(j in 1:NbZones){
  for(i in 1:NbCatAge){
weights1[ind_cat[, i] == 1, j] <- con_age[j, i] / ind_agg0[j, i]
    }
...
So, we can see that individual 3 (who's attributes can be viewed by entering `ind[3, ]`)
...
Often, however, integer weights are required. To estimate the individual-level
...
required. For this, the fractional weights must be integerised in a process
