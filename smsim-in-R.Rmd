...
# TODO: add IPF theory from Cambridge handout.pdf
# TODO: change integerisation section so it's part of IPF (done)
```
...
A wide range of methods can be used to allocate individuals to zones for
spatial microsimulation. As with the majority of statistical 
analyses, there are *deterministic* and *stochastic* methods.
...
In the literature, the divide between stochastic and deterministic methods
is usually mapped onto a wider distinction between *reweighting* methods
and *combinatorial optimisation* methods. Reweighting methods generally calculate
non-integer weights for every individual, whereas combinatorial optimisation
methods randomly allocate individuals from the survey into each zone, one-by-one.
If the fit between observed and simulated results improves after an individual
has been 'imported' into the zone in question, the individual stays. If the fit
deteriorates, the individual is removed from the zone and a different change
is attempted. The distinction between reweighting and and combinatorial optimisation
algorithms for creating spatial microdata is an important one: combinatorial
...
irrational: a quater of a person cannot possibly exist: either she is in the zone
or she is not!
...
and (via *population synthesis*) spatial microdata in 'long' format.
Likewise the synthetic spatial microdata generated directly via combinatorial
optimisation methods can be converted back in to a more *compressed* weight
matrix that links the individual and aggregate-level input datasets together
(hence the name *compression*). For this reason we have labelled this section
*reweighting algorithms* because all deterministic and stochastic
(or reweighting and cominatorial optimisation) approaches to the generation of
...
high weight (or be replicated many times in combinatorial optimisation approaches
--- it's essentially the same thing); individuals who are rare in a zone will
be given a low weight (or not appear at all, equivalent to a weight of zero).

```{r, fig.cap="Schematic of different approaches for the creation of spatial microdata encapsulating stochastic combinatorial optimisation and deterministic reweighting algorithms such as IPF. Note that integerisation and 'compression' steps make the results of the two approaches interchangeable, hence our use of the term 'reweighting algorithm' to cover all methods for generating spatial microdata.", fig.width=4, fig.height=4, echo=FALSE}
img <- readPNG("figures/co-vs-ipf-schema.png")
grid.raster(img)
```

If we had no information about the characteristics of a zone's inhabitants in SimpleWorld, for example, only its total population, we could consider that
the distribution of characteristics found in the sample is representative of the distribution of the whole population. In this case each individual in the zone would be chosen at random from sample and the distribution of individuals allocated to the zone would be
asymptotically (tending towards) that of the sample. In R, this case can
be achieved easily using the `sample()` command. We could use this method to
...
random number sequence should begin, ensuring repeatability. We must really trust the random function used. Note that it is impossible for a computer to
...
# Such as with the example of SimpleWorld, in each application have a matrix `ind` containing the 
# categorical value of each individual. `ind` is a two dimensional array (a matrix) in which each row represents an individual and 
# each column a variable. The value of the cell `ind(i,j)` is therefore the category of the individual `i` for the
# variable `j`. A second array containing the constraining count data `cons` can, for the purpose of explaining the theory, be
# expressed in 3 dimensions, which we will label `cons_t`: `cons_t(i,j,k)` is the number of individuals corresponding to the marginal 
# for the zone 'i', in the variable 'j' for the category 'k'. For example, 'i' could be a municipality, 'j' the gender and
...
# $$ weight(i,z,t+1) = weight(i,z,t) * \frac{consTheory(z,c,ind(i,c))}{ \sum_{j=1}^{#indiv} weights(j,z,t) * I(ind(j,c)==ind(i,c))}$$
...
The aim is to obtain the final weight matrix that will represent the representativity 
of each individual into each zone. Each row of this matrix is an individual and each column 
is a zone. The algorithm will operate zone per zone. So, the weight matrix will be filled in
...
zone. So, first we create an object, called *ind_agg0*, which rows are zones and columns are
the different categories of the variables. Then, we duplicate the weight matrix to keep in memory
each step.
...
than 50 years old and all weights are 1 for the moment. So, the new weight
...
individual are inside the matrix `ind_cat`. So, when multiplying `ind_cat`by 
the first column of weights1, we obtain a matrix which rows are individuals
and the value of each column is 0 if the person has not this characteristic
or the weight of the person in the other case. So, to aggregate this for the
...
and the constaints. This value is between -1 and 1 and in our case, 
...
(for example, when saving $\frac{4}{3}$, the computer can not save the infite of decimals and truncates the number).
...
machine by typing `.Machine$double.eps`). Remark that, instead of using `maxit` as stopping condition, 
we can use the `tol` argument which is a condition on the distance between two consecutive weight matrices.
...
`tol` alone is also hasardous. Indeed, if you have iteratively the same
matrices, but the approximations on the distance is a number bigger than 
your argument, the algorithm will continue. That's why we use both togeher and
...
required. For this, we have to possibily. First, we can consider the weights as
probability and randomly chose the indivuals in a distribution corresponding to the weights.
...
individual-level data back into `ints_df`, in a process we label
*population synthesis*, because it creates a synthetic population.
(Be aware that this term is used by transport modellers to represent
the entire process of spatial microdata generation.)
...
## Spatial microsimulation as an optimization problem

In general terms, an *optimization problem* consists in an function that has to 
be optimized, called *objective function*. This function is not necessary defined 
in the whole possible domain. So, the optimization problem has also constraints on
the arguments of the function. So, $x$ can be a vector $(x_1,x_2,..,x_n)$, so the objective function 
can have several arguments. The problem consists in finding arguments $x$ respecting
the constraints and that minimise the objective function. The expression of this
kind of problems is:
...
The parameters $par_i$ are defined and do not change. They determine the feasible
set for the variables. Indeed, for example, in aircraft, we want to minimise the
weight of the airplane, that depends on the shape (so, if it is an hypercube, all
dimensions of it) and the chose material. But, we have some constraint, because the
airplane has to resist to the pressure in the atmosphere and to be big enough to 
transport people. So, we search the shape and material (to include in $x$) that minimise
the weight ($f_0$ which is a single value depending on $x$) with the constraint that 
the volume (depending on the shape) has to be bigger than a fixed value ($par_1$) and 
the maximum force the airplane resists (depending on shape and material) 
must be bigger than the applicated force in the atmosphere ($par_2$). So, $par_i$ 
define a domain on which all value can evolve and then, we search inside this domain
the combination of $x_i$ that minimise the weight.
...
So, to use optimization problem, we have to define the problem and then, a lot of 
algorithm are already coded to find the result to the optimization problems. 
Intuitively, we search the number of occurencies we want of each individual to take
form the final population that fits the best the constraints. So, we want to 
find a vector of parameters `par` that will multiply the `ind` vector and we want the result
...
aggregate-level data. The measure of fit, so the distance, we use in this context is
