...
Conceptually, the big difference is that the R function **ipfp** uses an hard 
code written in C and can deal only with small problems as itself. With this 
...
```{r}
# Matrix which rows are individual and columns are zones
...
is : Which package is more efficient? The answer is hard to give, 
because it will depend on your application. We have seen that both 
...
to use **mipfp** (Seen in a coming section). Moreover, 
their results are also in different structures. Having weights for each 
individuals with **ipfp** and weights for the contingency table with **mipfp**
(that can be retranscribe in the other form if needed).

In conclusion to this comparison, **mipfp** is more adapted
and can deal with harder problems, but depending on the structure
of the data available, it can be easier for you to program with one
or the other package. In term of time, we compare here only for the
first zone of SimpleWorld problem. We compute only the function, 
since the code to threat the data before is very influenced by the 
application.
...
zone 1. It takes 0.001 second on a computer with a processor : 
Intel(R) Core(TM) i7-4930K CPU @ 3.40GHz. The code below make a similar analysis
...
# Measure time mipfp
system.time(Ipfp( weight_init, descript, target, tol = 1e-5))
```
...
In this case, the methods of population synthesis described in this
chapter must be adapted accordingly.
The challenge
