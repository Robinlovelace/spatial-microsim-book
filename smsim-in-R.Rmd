...
10 iterations. Note that 'convergence' in this context means that the
difference between the TAE in two consequtive iterations reaches the
...
to generate a result for every zone. This is analogous to the use of
`apply()` in conjunction with `ipfp()`.
...
The resulting array is larger than `weight_init`,
containing `n_zone` times more 
cells. To access all information about the first zone, 
...
is more practical and efficient. For many purposes
either strategy is valid; for large datasets the 'no for loop' strategy
may be appropriate.
