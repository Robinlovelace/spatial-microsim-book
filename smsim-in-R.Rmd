...
The R package  **mipfp** is more recent than **ipfp**
and a more generalized version of the IPF algorithm.
Indeed, **ipfp** generates a two-dimensional weight matrix based on mutually
exclusive (non cross-tabulated) constraint tables.
This is useful for applications without cross-tabulated constraints is not available.
**mipfp** allows for multiple cross-tabulations in the constraint variables,
...
```{r, echo=FALSE}
# Original:
# problems of the form $Ax=b$.
# Changed because the meaning of Ax=b has not been defined
```
...
To illustrate the use of `Ipfp`, **mipfp**'s only function,
we will create a fictive example. 
...
Note that the population is equal in each constraint (50 people). Note also
...
Now all constraint variables have been defined. Let us now define 
...
Everything being defined, we can execute
...
The addition of a constraint means there are fewer possible outcomes, implying that the algorithm 
...
```{r, echo=FALSE}
# Please MD, show mipfp can solve the SimpleWorld problem (RL)
```
...
In this case, the method of population synthesis must be adapted.
The challenge
is to generate the individual-level data 
that fits all the constraint tables. However, many
combinations of individual-level data
could correspond to these distributions. Depending on the aim
of the spatial microsimulation, finding one can be sufficient.
Another consideration is that of *entropy maximisation*.
This concept consists in considering the population that
represents the most micro-level populations (Bierlaire, 1991).
For example, if a person has exactly the same charcteristics of 
one in other zone, exchange these two people do not change 
the synthetic population.
This solution is considered in the subsequent chapter, which
explainings how to 
allocate individuals from the spatial microdata into households to fit
household-level constraints.

There are a huge possibility of combinations of possible available data.
We could not develop all cases in a book. We will here present
two possible alternative data when having no individual-level 
datasets.

### Global cross-tables and local marginal distributions

We suppose here to have the whole contingency table in the most 
aggregated data. Note that this cross-table could be the result of a 
...
no additional databases, the single possibility is to 
rescale the global cross-table for each zone. Note that this
implicitly implies stating the correlations between the variables
...
```{r}
# aggregation of individual-level dataset
table(ind$age, ind$sex)

# total population in each zone
consTot <- cbind(cons, rowSums(cons) / 2) # add total column
colnames(consTot) <- c(colnames(cons), "Total") # name the new column
consTot # print the extended table
```
...
GlobalCons <- table(ind$age, ind$sex)
GlobalCons[1,] <- c(6,9)
GlobalCons[2,] <- c(7,11)
...
When having only these data available, the best way to 
...
resNames <- list(1:length(LocalCons), rownames(GlobalCons), colnames(GlobalCons))
res <- array(0, dim=c(length(LocalCons), dim(GlobalCons)), dimnames=resNames)

# re-scale the cross-table to fit the zone's constraints
# loop on the zones
for (zone in 1:length(LocalCons)){
  res[zone,,] <- GlobalCons*LocalCons[zone]/sum(GlobalCons)
}
...
The fractional result needs an additional step to be really effective.
If using simply the round function gives a satisfying result, in terms
of fitting the constraints, we can use this simple alternative. However, 
in the other case, you can process to an adaptation of the proposed by
Lovelace and Ballas (2012) : *truncate, replicate, sample* (TRS).
This method can not be followed exactly, because we want to perfectly fit 
the few constraints we have. In our example, the result is good by using 
the round function, as we can see thanks to the code below.
...
# (Good if all TRUE)
for (zone in 1:length(LocalCons)){
 print( sum(round(res[zone,,])) == LocalCons[zone] )
}

# Calculate the total simulated cross-table
SimTot <- array(0, dim=dim(GlobalCons), dimnames=c(rownames(GlobalCons), colnames(GlobalCons)))
for (zone in 1:length(LocalCons)){
...
# (Good if 0)
sum(SimTot - GlobalCons)
```

To explain the proposed method to adapt TRS, we will integerise
our results also with the second process. The first stage is to 
...
sum(LocalCons) - sum(resTruncate)

# Calculate the total simulated cross-table
SimTotTruncate <- array(0, dim=dim(GlobalCons), dimnames=c(rownames(GlobalCons), colnames(GlobalCons)))
for (zone in 1:length(LocalCons)){
 SimTotTruncate <- SimTotTruncate + resTruncate[zone,,]
}

# number of missing individuals per category
ToAdd <- GlobalCons - SimTotTruncate
ToAdd

# number of missing individuals per zone
for (zone in 1:length(LocalCons)){
 print(LocalCons[zone] - sum(round(resTruncate[zone,,])))
}

```

The principe will be to add people in the not completed zones and
...
while (sum(LocalCons) - sum(resTruncate) >0){
  ...
  if (LocalCons[index[1]]-sum(resTruncate[index[1],,]) > 0){
    resTruncate[index]<-resTruncate[index]+1
  }
}
```

The new values in `resTruncate` follows all constraints. 
...
The solution proposed by Barthélemy and Toint (2012) is intuitive and we 
...
The pseudo-code below corresponds to the code done by Barthélemy and Toint (2012).
