...
'i' could be a municipality, 'j' the gender and 'k' the female. So, 
...
So, 'weight(i,j)' corresponds to the weight of the individual 'i' in the 
zone 'j'. For the zone 'z', we will adapt the weight matrix to
...
of the individual 'i' for the variable 'c'. So, the denominator corresponds 
...
integer weight, we do not have the problem of IPF, for 
which the perfect fit consists in non integer weights and the
integerisation has to be really effective or could decrease the
fit to the constraint. The fact that the gradient contains "NA"
in the end of the algorithm is not a problem, because it just means
that they have not been calculated.

Remarks also that there can be several solution performing 
a perfect fit. This phenomena depends on the seed chosen for
the random draw. Indeed, if we chose now the seed 0
(by writing `set.seed(0)`), as before, we obtain the weights 
`(0, 6, 4, 2)` that results also in an perfect fit for zone 1:

```{r}
colSums(ind * c(0 ,6 ,4 ,2))
```

These two potential synthetic populations reach a perfect fit, but 
are pretty different. Indeed, we can observe the two populations:
...
With this method, there can not be a population with 1 male of
