...
We can verify that the population is equal for each constraint (50 people).
Note also
the order in which each category is encoded. This is a common source of
error in population synthesis, explaining the naming of values with
`names(...) <- ..` .
The constraints are the target marginal totals and must be stored as a list.
To tell the algorithm 
which elements of the list correspond to which constraint,
a second list with the description 
of the target is created (see `descript` below). We print and find the
sum of *target* to verify that we have encoded everything well.

```{r}
target <- list(sex, age, diploma)
descript <- list(1, 2, 3)
target
sapply(target, sum) # check the population totals
```

With constraints correctly encoded, let us define 
the `seed`, the initial array to be updated.
This array represents the survey microdata, equivalent to `ind` in the 
SimpleWorld example described above. 
This matrix must have dimensions equal to the number of categories in the constraints:
$2 \times3 \times 4$ in this case. Each cell of the array represents 
a combination of the attributes' values, and thus defines a particular 
category of individuals. A cell value of 0 means that the category is not possible or that an individual with the corresponding combination of attributes does not
exist. In our example there are no individuals under 18  years old with a 
diploma level higher than 2, as such a combination is impossible. 
...
seed <- array (1, c(2,3,4), dimnames = names)
seed[, c("Less18"), c("Level3","Level4")] <- 0
```

With everything defined, we can execute
`Ipfp()`. The function *requires* 3 arguments: 

- `seed`: a multi-dimensional array representing the individual-level
  microdata. This is represented by the `seed` object,
  created and described above. 
- `target.list`: a list of the order of target margins, in the `target.data`
  (represented by the `descript` object).
- `target.data`: the target marginal totals to be approximated, represented by
  a list (this is the `target` object created above).

As with `ipfp()`, `mipfp()` takes additional *optional* arguments,
the full details of which can be viewed by entering `?mipfp`.
Stopping criteria can be set by `tol` and `maxit`.
Setting `print = TRUE` makes `Ipfp` display the result after each iteration.
...
array, in addition to information about the convergence of the algorithm. 
Note that if the target margins are not consistent, the input datasets are
...
The output shows that the `TAE` between each iteration reached `tol` after 8 
...
```{r}
# Print the result TODO: uncomment when new version of mipfp is used
# result$xi.hat 
# Check the total number of persons
# sum(result$xi.hat)
```
...
When cross-tabulated constraints containing the same variable
(e.g. age/sex and age/mode both contain age) are used, we must 
check for consistency
(otherwise the result may never converge). 
In this case can display the margins of `cross` along with `diploma` and `age`
to validate the consistency assumption:
...
The addition of a constraint leave less freedom to the algorithm,
implying that the algorithm 
converges faster. By printing the results,
we see that all constraints are respected and
...
# Idea: this section could go in a separate chapter or section comparing different
...
weight_mipfp <- Ipfp( seed, descript, target, print = TRUE, tol = 1e-5)
...
for (zone in 1:NbZones){
  ...
  res <- Ipfp( seed, descript, target, tol = 1e-5)
...
seed_no_income <- table( ind[, c(2,3)])

# Measure time mipfp
system.time(Ipfp(seed_no_income, descript, target, tol = 1e-5))
```
...
# Equitable calculus - remove income 
seed_no_income <- table(ind[, c(2,3)])*5000
...
```{r, echo=FALSE}
# Should this be a separate chapter? Perhaps...
```

In each of the previous sections we have demonstrated how to generate
fractional weights. In the real world, however, entities are often present
only in multiples of one. It is impossible, for example, for $2\frac{3}{4}$
people to inhabit a zone. However, according to the methods described above,
it is! To convert the *weight matrices* generated above into counts of whole
individuals, we must implement a two step process which,

1. converts the fractional weights into whole numbers (integerisation)
2. replicates repeated individuals multiple times for each zone (expansion)

It is the purpose of this section to describe each of these processes in turn.

### Integerisation {#sintegerisation}

```{r, echo=FALSE}
# TODO: update this section
# Morgane: I do not understand the second option so I've commented this paragraph
# for the time being - I hope to uncomment it soon!
# 
# Integerisation is the process of converting
# a weight matrix containing non-integer numbers
# (technically described as the *double* data type --- this can be ascertained
# by using `is.double()`) into integer counts. 
# This can be done using one of two approaches.
# First, we can consider the weights as probabilities and inside each zone
# we can randomly draw the number of individuals needed (approximating the
# constraint of
# the zone). This random draw is made inside a distribution defined
