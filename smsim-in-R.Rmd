...
The R package  **mipfp** is more recent than **ipfp** . The concept is exactly the same as **ipfp**, but it 
is a generalized version. Indeed, **ipfp** requires two constraints and generates a two-dimensional weight matrix.
...
Now, with the **mipfp** package, you can fill a $N$-dimensional weight matrix and have as constraints some marginal table
or contingency table. Thus, with this function, you can easily use your own data and adapt the procedure to your own 
application. The first step is always to load the package (after its installation on your computer):

```{r}
library(mipfp) # after install.packages("mipfp")
```

To illustrate the use of this function, we will create a fictive example. The case of SimpleWorld is here
to simple to really illustrate the power of this package. Suppose, for each spatial
zone, we need to have for each people his age, sex and diploma level. The sex has two categories (Male and Female),
the age is classified in three categories (less than 18, 18-50 and more than 50) and the diploma level is
divided in 4 different levels. We consider a zone with 50 inhabitants. The classical spatial microsimulation 
problem consists in having all marginal distribution and the cross table only for a sample. 

We will consider the variable in the order (sex,age,diploma). Our constraints could for example be:
...
For the moment, all constraint are defined. Let now initialise the weight matrix. The dimension
of this matrix has to be $(2 \times3 \times 4)$. In our case, we will consider that the weight
matrix contains 0 when the category is not possible. For example, being less than 18 years
old and having a diploma above level 2 is impossible. The other categories will be initialised to 1
...
When having several constraints concerning the same variable, we have to be careful. The targets have
to be pertinent. We can display the margins of the cross table and the previous target. There are
...
As with the **ipfp** package, this results in a non-integer matrix and we can determine
a final synthetic population even by considering the weights as probabilities or by proceeding
to the integerisation and expansion steps defined previously.
