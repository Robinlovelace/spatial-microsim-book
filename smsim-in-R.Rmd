...
data is available for population synthesis. Clearly, depending
of the domain, there are a lot or few datasets available. Moreover, 
if aggregation needed play a role also and the aim of the research. 
Thus, each research has to adapt his spatial microsimulation to 
the data and the aim. In some cases, the data are recorded, but it 
includes personal information about specific humans and the privacy
law defines these data as confidential.

Usually, having the aggregated distributions is possible. The big deal
is to find and access the individual-level data, even if they don't concern
the whole population. When having only aggregated data, you have to 
reach a population that fits all these constraints. However, several 
populations could correspond to these distributions. Then, you can be satisfied
by one feasible result. Another consideration could be that from all 
feasible result, we take into account the one that maximises the entropy of
the model. This solution is considered in the chapter explaining how to 
join the people into households in our spatial microsimulation.
