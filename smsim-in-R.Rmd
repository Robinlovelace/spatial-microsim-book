...
This process of swapping or 'switching' individuals is considered in the subsequent chapter, which
explainings how to 
...
resNames <- list(1:nrow(cons), rownames(global_cons), colnames(global_cons))
res <- array(0, dim=c(nrow(cons), dim(global_cons)), dimnames=resNames)
...
spatial microdata. The `round()` function gives a satisfying result, in terms
...
SimTot <- array(0, dim=dim(global_cons), dimnames=c(rownames(global_cons), colnames(global_cons)))
for (zone in 1:length(total_pop)){
...
# Calculate the total simulated cross-table
SimTotTruncate <- array(0, dim=dim(global_cons), dimnames=c(rownames(global_cons), colnames(global_cons)))
for (zone in 1:length(total_pop)){
