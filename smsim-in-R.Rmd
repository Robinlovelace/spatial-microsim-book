...
# TODO: update figure with expansion

# TODO tasks completed:
# TODO: change integerisation section so it's part of IPF (RL)
```
...
and combinatorial optimisation algorithms for creating spatial microdata is an important one: combinatorial
...
combinatorial optimisation approaches); individuals who are rare in a zone will
...
performed, we run it on the same input vectors:
...
the individual that is allocated a weight of 2 (individual 2 in this case)
...
for the entire domain of possible inputs. Optimization problems can be
*unconstrained* or *constrained*, by
limits on the values that arguments of the function can take
([Boyd 2004](http://stanford.edu/~boyd/cvxbook/)). Under this framework,
...
the value of the objective function is $f_0(x,par)$, depends on $x$, where
*par* is a vector of predefined arguments or parameters of length $i$
($par_1,par_2,..,par_i$). 
This
...
the parameters $par_i$ represent the aggregate constraint variable
`cons` which do not change. $x$ represents the weights which will
be calculated to minimise
to minimise $f_0(x)$, subject to $par$.

Indeed, for example, in aircraft, we want to minimise the
...
must be greater than the force applied by the atmosphere ($par_2$). So, $par_i$ 
...
reachable with a computer perfomung calculus) is observable after only the first iteration.
...
Note that these results are biaised by the example that is pretty small 
