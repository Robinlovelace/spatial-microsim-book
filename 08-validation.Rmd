...
- Chi-squared, a statistical hypothesis test that compare two hypothesis and determine which one is more
...
the number of variable^[This is very intuitive, since
...
$$
MRE = sum_{i=1}^{n_var*n_zones}RE_{restricted to iteration i (defining zone and var)}
$$
...
$$
\chi^2= \sum_{i=1}^{n_line}\sum_{j=1}^{n_column}\frac{(sim_{ij} - obs_{ij})^2}{obs_{ij}}
$$
...
is around 0.9816. This value becomes already very closer to 1.
...
Indeed, an error for 26445 persons multiplied by variables out of 4,871,397 is not a so big error. For this reason, 
the relative error RE (also called the standardised absolute error) is often preferable. We observe a RE of 
only 0.54%. Note that RE is simply TAE divided by the total of all observed cell
...
in term of total population,
it can happen having more differences between the two maps. Indeed, with the same 
...
Only few observed individuals were not owning a car. Thus, for zones needing a lot of persons
...
cons[best, c("Car", "NoCar")] # constraint for 3 best zones
```
...
We have explored methods of checking that the results of the
CakeMap spatial microsimulation model correspond to reality.
This has primarily involved checking
