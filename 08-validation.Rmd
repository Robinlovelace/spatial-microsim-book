...
@Voas2001 investigated the matter more
...
```{r}
cor(TAEVec, SAEVec) # the measures are highly correlated
```
...
```{r, echo=FALSE, fig.cap="Geographical distribution of Total Absolute Error (TAE) and Standardised Absolute Error (SAE). Note the zones of high error are clustered in university areas such as near the University of Leeds, where there is a high non-resident population.", message=FALSE, warning=FALSE, results=F, fig.height=14, fig.width=9}
...
wards$`Sqrt(SAE)` <- sqrt(wards$SAEVec)
# p2 <- tm_shape(wards) +
#   tm_fill("Sqrt(SAE)", n = 3) +
#   tm_borders()
...
tm_shape(osm_tiles) +
  tm_raster() +
tm_shape(wards) +
  tm_fill(c("Sqrt(TAE)", "Sqrt(SAE)"), n = 3, alpha = 0.5) +
  tm_borders()

# Add SAE map - commented out due to error with scale_transform
...
This process, generally reffered to as 'internal validation',
