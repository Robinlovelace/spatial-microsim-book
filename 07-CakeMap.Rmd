...
So for the book has explained what spatial microsimulation
...
-*Selection of constraint variables* ([](#CakeMapConVar)) describes and justifies
the choice of the constraint variables on this specific example.
-*Preparing the input data* ([](#CakePrep)) contains the pieces of code
necessary to load and prepare the data, before to perform the spatial microsimulation.
-*Using the `ipfp` package* ([](#CakeMapIPFP)) develops the whole process
to create a synthetic population thanks to the R package **ipfp**.
-*Using the `mipfp` package* ([](#CakeMIPF)) includes also the whole
process, but with the R package **mipfp**.
-*Comparing methods of reweighting large datasets* ([](#CompareMeth)) compares
...
As discussed in ([](#data-prep)), we must first decide which variables 
...
aggregate-level datasets can be chosen from. Suppose our aim is to analyse the consumption 
...
```{r, fig.align='center', fig.cap="Example of raw aggregate-level input data for CakeMap aggregate data, downloaded from http://infuse.mimas.ac.uk/.", fig.scap="Example of raw aggregate-level input data", echo=FALSE}
library(jpeg)
...
```{r, echo=FALSE, fig.cap="Scatter plot of the relationship between observed cell counts for all categories and all zones from the census (x axis) and the simulated cell counts after IPF (y axis).", fig.scap="Scatter of simulated and desired counts with ipfp",fig.align='center'}
...
(sex and age^[or age $\cross$ sex, as cross-tabulations are often represented].
...
```{r, echo=FALSE, fig.cap="Scatter plot of the relationship between observed cell counts for all categories and all zones from the census (x axis) and the simulated cell counts after IPF (y axis).", fig.scap="Scatter plot of simulated and desired counts with mipfp", fig.align='center'}
...
```{r, echo=FALSE, fig.cap="Scatter plot of the relationship between observed cell counts for all categories and all zones from the census (x axis) and the simulated cell counts after IPF (y axis).", fig.scap="Scatter plot of simulated and desired counts (ipfp vs mipfp)", fig.align='center'}
layout(matrix(c(1:3,3), 2, 2))
...
rapidly have better performances.
