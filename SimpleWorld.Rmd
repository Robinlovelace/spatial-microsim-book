...
come from the SimpleWorld census, so we can consider that the planet's entire
population is represented we want to analyse.
However, we know only the marginal distributions
of the two variables, but not the contingency table or cross-tabulation
that links age and sex. This means that we have per-zone information on
the number of young and less young people and the number of males and females.
...
So, we are not satisfied with this geographic count data alone. 
These contexts, where we have access to geographical count data (usually from a
census of population) and a more detailed but non-geographical individual-level dataset
that spatial microsimulation is useful.
It is in this context, where we would like to impute missing information
about geographic zones, that we turn to spatial microsimulation
to better understand the population of SimpleWorld.
To do this, we need some additional information: an individual-level *microdataset*.
